...
class User:
    ...
    # Should the node drop content when he is out?
    def shouldDrop(self,c,z):
        remaining = self.max_time_elapsed - self.time_elapsed[z.id]
        i = c + remaining
        self.drop = True
        if len(self.myFuture) < i:
            if self.myFuture[c] != self.myFuture[i] and self.myFuture[i] != -1:  
                self.drop = False
        return self.drop
